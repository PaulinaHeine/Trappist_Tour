...
#für eine oder mehrere Permutationen fonts und lösungen ausgeben
...
def combine_scores(points):
    """ Function for aggregating single solutions into one score using hypervolume indicator """
    import pygmo as pg
    ref_point = np.array([25000000, 40000000])
...
scores = [[] for i in range(len(perms))]
...
for n in range(len(perms)):
    ...
    for w in range(len(f)):
        print(combine_scores(np.array([f[w]])))
        if combine_scores(np.array([f[w]])) < 0:
            #scores.append([f[w],r[w]])
            print(combine_scores(np.array([f[w]])))
            print(f[w], r[w])


    # scores.append([perms[n], f, r, print(combine_scores(np.array(f)))])
    # print(combine_scores(np.array(f)))
    # print(f'{n} done')

#Für jede einzelne Permuattion die font augeben lassen

    plt.figure(figsize=(7, 5))
    plt.scatter(f[:, 0], f[:, 1], s=40, facecolors='none', edgecolors='blue')
    ref_point_x = 2500
    ref_point_y = 4000
    plt.scatter(ref_point_x, ref_point_y, color="red")
    plt.title(f"permutation:{list(perms[n])}")
    plt.xlim(-5000, 200000)
    plt.ylim(0, 10000)
    plt.show()
    # plt.savefig(f'/Users/paulinaheine/a.Uni/Semester 6/Bachelorarbeit/Code/choosen_perms/Graphs/{n},permutation:{list(perms[n])}')
