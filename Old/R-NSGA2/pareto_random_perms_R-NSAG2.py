...
start = time.time()
# Hier werden jenadchem welche/wieviele Permutatuonen geprüft werden sollen, per pymoo die idealen Lösungen ausgegebne.
# Diese werden mit ihren scores gespeichert. Die dominierten Lösungen werden gelöscht. am ende wird eine Grafik ausgegebn


# determine alle permutations
# perms = (list(itertools.permutations([0, 1, 2, 3, 4, 5, 6])))
# perms = perms[0:5]
# perms = random.sample(perms, 300)
...
# Boundaries
upper_b = udp.get_bounds()[1][:-7]

lower_b = udp.get_bounds()[0][:-7]

# 27 variables
t_delta = []
conts = []

# time and delta v of best solutions
f_of_best = []

# Schleife, die auf alle gegebenen perms den NSGA2 anwendet
...
for n in range(len(perms)):
    ...
    ref_points = np.array([[0, 0], [3500, 5000], [2500, 4000], [0, 4000], [2500, 0]])  # 84
    # ref_points = np.array([[0, 4000], [2500, 0]]) 82
    # ref_points = np.array([[0, 0], [4500, 6000]]) 85
    # ref_points = np.array([[0, 0], [2500, 4000]]) 83
    # gehen auch mehr als 2???????
    # Get Algorithm
...
print(indices_2, sorted(indices_2, reverse=True))


# alle dominierten punkte Löschen
...
f_of_best = np.array(t_delta)

#alle punkte hinter dem refpunkt raus
final_scores = []
for t in range(len(f_of_best)):
    final_scores.append(combine_scores(np.array([f_of_best[t]])))

for v in reversed(list(range(len(final_scores)))):
...
print(f_of_best)
print(combine_scores(np.array(f_of_best)))


# beste permutationen raussuchen
best_perms = []
for x in range(len(conts)):
    best_perms.append(conts[x][1])
best_perms = list(set(best_perms))

stop = time.time()
time_interval = stop - start
print(time_interval / 60)


t_delta = np.array(t_delta)

plt.figure(figsize=(7, 5))
plt.scatter(t_delta[:, 0], t_delta[:, 1], s=40, facecolors='none', edgecolors='blue')
ref_point_x = 2500
ref_point_y = 4000
