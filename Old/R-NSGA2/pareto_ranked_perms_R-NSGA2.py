...
#Wenn random permns geprüft werden sollen, bis 400 auskommentieren und oben random aktivieren
rank_perms = []
for p in range(len(perms)):
    example = udp.example()[:-7]
    example += perms[p]
    points = udp.fitness(example)
    s = combine_scores(np.array([points[:2]]))
    rank_perms.append([perms[p], s])

rank_perms = sorted(rank_perms, key=itemgetter(1))

perms = []

for x in range(6):
    perms.append(list(rank_perms[x][0]))
...
for n in range(len(perms)):
    ...
    ref_points = np.array([[0, 0], [2500, 4000]])
    ...
    for t in range(len(f)):
        # scores.append(f[t][0] + f[t][1])
        scores.append(combine_scores(np.array([f[t]])))
...
print(f_of_best, scores)
print(combine_scores(np.array(f_of_best)))



plt.figure(figsize=(7, 5))
plt.scatter(f_of_best[:, 0], f_of_best[:, 1], s=40, facecolors='none', edgecolors='blue')
ref_point_x = 2500
ref_point_y = 4000
plt.scatter(ref_point_x, ref_point_y, color="red")
plt.title("Pareto font rnsga2_ranked")
